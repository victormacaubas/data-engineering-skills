...
Load **in addition to** `javascript-typescript.md` when the scope contains `.jsx`/`.tsx` React components or hooks. This pack adds React-specific footguns on top of the JS/TS pack; the dimension keys in parentheses match `review-dimensions.md`. Read both before scoring.
...
## Correctness & hidden bugs (`correctness`, `resource-lifecycle`, `concurrency`)
...
- **Missing cleanup** (`resource-lifecycle`): `useEffect` that subscribes/sets a timer/opens a socket without returning a cleanup → leak + "setState on unmounted component"; a fetch in an effect with no abort on unmount (race: late response overwrites newer state).
...
- Race in concurrent fetches where the slower response wins (`concurrency` — no request-id/abort guard).
...
- A `useEffect` that subscribes/opens a resource with no cleanup is **high** under `resource-lifecycle` — it leaks and can setState after unmount.
- A stale-closure bug in a handler that silently uses old state is **high** under `correctness` (silent-wrong-behavior).
