...
Load when the review scope contains `.tf` or `.tfvars` files. This pack sharpens the universal review dimensions for infrastructure-as-code; the dimension keys in parentheses match `review-dimensions.md`. Read it fully before scoring.
...
## Security (`security`) — weighted heavily for IaC
...
- A `0.0.0.0/0` ingress on an admin/DB port, a hardcoded secret, or a `"*":"*"` IAM policy is **critical** under `security` — it's a live exposure, not a latent risk.
...
- `count` over a list of stateful resources (index-shift destroy hazard) is **high** under `correctness`.
...
- A referenced-but-undeclared CloudWatch log group is **medium** under `observability` (silent logging failure + unbounded-cost / no-retention risk), unless clearly owned by another module (then a summary note).
- Unpinned provider/module versions is **medium** under `dependencies` (reproducibility), **high** if the module is widely consumed.
