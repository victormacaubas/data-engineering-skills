...
The raw load databases sometimes use versioned schema names (e.g., `ADVISOR_DASH_V1` instead of `ADVISOR_DASH`). The downstream views in `PROD_SOURCE_DB` abstract this away, so users query `prod_source_db.advisor_dash.clients` but the base table is actually `PROD_ENT_LOAD_DB.ADVISOR_DASH_V1.CLIENTS`.
...
Database roles are scoped per-database. A role named `CLOCKWORK_SCHEMA_RO` in `PROD_SOURCE_DB` is a **different** role than `CLOCKWORK_SCHEMA_RO` in `PROD_ENT_LOAD_DB`. When masking policies on raw tables use `IS_DATABASE_ROLE_IN_SESSION()`, they check for the role in the raw load database — not in PROD_SOURCE_DB.
