...
2. **Schema names may differ between layers.** Raw schemas sometimes have a `_V1` suffix (e.g., `ADVISOR_DASH_V1`). Use ACCESS_HISTORY's `BASE_OBJECTS_ACCESSED` to find the actual underlying table.
...
4. **Database roles are scoped per-database.** Having `CLOCKWORK_SCHEMA_RO` in PROD_SOURCE_DB does NOT satisfy a masking policy that checks for that role in PROD_ENT_LOAD_DB.
