...
| `code-auditor` | `claude-opus-4-6[1m]` | Use to find defects in code in any language — a file, module, PR, or branch diff — security holes, correctness bugs, performance problems, weak error handling. Runs the code-audit skill end-to-end and returns the JSON artifact path plus the headline verdict and score. Findings ranked by severity and anchored to excerpts; does not assess the shape of the change. |
