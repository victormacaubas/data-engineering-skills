...
4. **Implement, task by task.** Write code and tests for each task in your slice, in order. For non-trivial Python, load and follow the installed `python-engineering-standards` skill when available; otherwise follow the repository's Python conventions and record that the skill was unavailable in `Decisions made`. Keep each task's changes coherent so the orchestrator can review them per-task.
