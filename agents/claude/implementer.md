...
4. **Implement, task by task.** Write code and tests for each task in your slice, in order. Follow the pinned `craft:python-engineering-standards` skill for all non-trivial Python. Keep each task's changes coherent so the orchestrator can review them per-task.
